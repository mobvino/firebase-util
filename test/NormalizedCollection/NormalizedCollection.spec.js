...
describe('NormalizedCollection', function() {
  describe('<constructor>', function() {
    it('should accept a query as a parameter', function() {
      ...
      expect(function() {
        var ref = new NormalizedCollection(q, r).select('foo.string', 'bar.number').ref;
      }).not.toThrowError();
    });
    ...
    it('should accept a NormalizedRef as a parameter', function() {
      ...
      expect(function() {
        var ref = new NormalizedCollection([nc1, 'join'], fb.child('baz')).select('join.string', 'baz.number').ref;
      }).not.toThrowError();
    });
  });
});